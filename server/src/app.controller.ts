...
export class AppController {
  constructor(
    ...
    private readonly spoonapiService: SpoonapiService,
  ...
  async getRecipes() {
    ...
    value
      .pipe(map((resultSet) => resultSet.results))
      .subscribe(async (recipes) => {
        for (let i = 0; i < recipes.length; i++) {
          ...
          r.id = recipes[i].id;
          console.log(r);
          await this.spoonapiService
            .getRecipeNutrition(Number(r.id))
            .subscribe(async (nutritions) => {
              r.calory = parseInt(nutritions.calories);
              r.carbohydrates = parseInt(nutritions.carbs);
              r.fats = parseInt(nutritions.fat);
              await this.spoonapiService
                .getRecipeInfo(recipes[i].id)
